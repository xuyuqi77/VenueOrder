...
(function(a) {
    ...
    a("head", c).append('<link href="' + m + "skins/" + o + '.css" rel="stylesheet" type="text/css"/>'),
        ...
        a.dialog = function(l) {
            var m = this,
                n, o, q, r, s, t, u, v, w, x = m.opt = a.extend({
                ...
                G = function(d) {
                    ...
                    x.html && x.html.nodeType && (a(o).append(x.html), x.html.style.display = ""),
                        v = [window],
                    b != window && v.push(b),
                    x.page && (m.dgFrm = a("#lhgfrm_" + x.id, c)[0], x.link || (m.dgWin = m.dgFrm.contentWindow, m.dgFrm.lhgDG = m), a(m.dgFrm).bind("load",
                        function() {
                            this.style.display = "block";
                            if (!x.link) {
                                var b = a.browser.msie ? m.dgWin.document: m.dgWin;
                                a(b).bind("mousedown", M),
                                    v.push(m.dgWin),
                                    m.dgDoc = m.dgWin.document,
                                x.autoSize && I(),
                                a.isFunction(x.dgOnLoad) && x.dgOnLoad.call(m)
                            }
                            n.style.display = "none"
                        })),
                    x.xButton && x.titleBar && a(q).bind("click", x.onXclick || m.cancel),
                    x.maxBtn && x.titleBar && (a(y).bind("click", m.maxSize), a(r).bind("dblclick", m.maxSize)),
                    x.minBtn && x.titleBar && a.isFunction(x.onMinSize) && a(B).bind("click", x.onMinSize)
                },
                H = function(a) {
                    var b = r.offsetHeight,
                        c = s.offsetHeight,
                        d = x.btnBar ? u.offsetHeight: 0,
                ...
                I = function() {
                    var a = r.offsetHeight,
                        b = s.offsetHeight,
                        c = x.btnBar ? u.offsetHeight: 0,
                        d = s.offsetWidth * 2,
                        e,
                        f,
                        g;
                    if (x.html) e = Math.max(o.scrollHeight, o.clientHeight || 0),
                        f = Math.max(o.scrollWidth, o.clientWidth || 0);
                    else if (x.page && !x.link) {
                        if (!m.dgDoc) return;
                        g = h(m.dgDoc),
                ...
                K = function(b) {
                    ...
                    function o(a) {
                        ...
                        h = {
                            ...
                            c = b,
                        x.rang && (h.x < n.x && (h.x = n.x), h.y < n.y && (h.y = n.y), h.x > d && (h.x = d), h.y > g && (h.y = g)),
                            m.dg.style.top = x.fixed && !f ? h.y - n.y + "px": h.y + "px",
                            m.dg.style.left = x.fixed && !f ? h.x - n.x + "px": h.x + "px"
                    }
                    var c, d, g, h, i = v,
                        l, n;
                    a(b).bind("mousedown",
                ...
                L = function(b) {
                    ...
                    function o(a) {
                        ...
                        i.w < 200 && (i.w = 200),
                        i.h < 100 && (i.h = 100),
                            m.dg.style.top = x.fixed ? g.y - n.y + "px": g.y + "px",
                            m.dg.style.left = x.fixed ? g.x - n.x + "px": g.x + "px",
                ...
                O = function() {
                    m.dgFrm && (x.link || a(m.dgFrm).unbind("load"), m.dgFrm.src = "about:blank", m.dgFrm = null),
                    x.html && x.html.nodeType && (a(m.curDoc).append(x.html), x.html.style.display = "none"),
                    x.autoPos && a(b).unbind("resize", N),
                        v = [],
                        a(m.dg).remove(),
                        m.dg = null,
                        A = !1,
                        z = {},
                        n = o = q = r = s = u = t = y = B = null
                };
            m.ShowDialog = function() {
                if (a("#lhgdlg_" + x.id, c)[0]) return m;
                x.cover && E(),
                x.fixed && (x.maxBtn = !1, x.minBtn = !1);
                var d = x.fixed && !f ? "fixed": "absolute",
                    e = D();
                m.dg = a(e, c).css({
                    position: d,
                    zIndex: i()
                }).appendTo(c.body)[0],
                    n = a("#lhgdg_load_" + x.id, c)[0],
                    o = a("#lhgdg_inbox_" + x.id, c)[0],
                    q = a("#lhgdg_xbtn_" + x.id, c)[0],
                    r = a("#lhgdg_drag_" + x.id, c)[0],
                    s = a("#lhgdg_drop_" + x.id, c)[0],
                    u = a("#lhgdg_btnBar_" + x.id, c)[0],
                    t = a("#lhgdg_content_" + x.id, c)[0],
                    y = a("#lhgdg_maxbtn_" + x.id, c)[0],
                    B = a("#lhgdg_minbtn_" + x.id, c)[0],
                    H(m.dg),
                    F(m.dg, x.top, x.left, x.fixed),
                    G(m.dg),
                    x.drag ? K(r) : r.style.cursor = "default",
                    x.resize ? L(s) : s.style.cursor = "default",
                f && a(s).css("ie6png") && a(m.dg).fixie6png(),
                x.btnBar && x.cancelBtn && m.addBtn("dgcancelBtn", x.cancelBtnTxt, x.onCancel || m.cancel),
                x.html && (n.style.display = "none", o.style.display = "", x.autoSize && I()),
                x.timer && m.closeTime(x.timer, x.autoCloseFn),
                x.html && a.isFunction(x.dgOnLoad) && x.dgOnLoad.call(m),
                x.autoPos && a(b).bind("resize", N)
            },
                ...
                m.maxSize = function() {
                    ...
                    b = k(),
                        c = j(),
                        A ? (m.reDialogSize(z.dgW, z.dgH), a(m.dg).css({
                            top: z.dgT,
                            left: z.dgL
                        }), a(y).addClass("lhgdg_maxbtn").removeClass("lhgdg_rebtn"), x.drag && (K(r), r.style.cursor = "move"), x.resize && (L(s), s.style.cursor = "nw-resize"), A = !1) : (z.dgW = m.dg.offsetWidth, z.dgH = m.dg.offsetHeight, z.dgT = m.dg.style.top, z.dgL = m.dg.style.left, a(m.dg).css({
                            top: c.y + "px",
                            left: c.x + "px"
                        }), m.reDialogSize(b.w, b.h), a(y).addClass("lhgdg_rebtn").removeClass("lhgdg_maxbtn"), x.drag && a(r).unbind("mousedown").css("cursor", "default"), x.resize && a(s).unbind("mousedown").css("cursor", "default"), A = !0)
                },
                ...
                m.addBtn = function(b, d, e, f) {
                    f = f || "left";
                    if (x.btnBar) if (a("#lhgdg_" + x.id + "_" + b, c)[0]) a("#lhgdg_" + x.id + "_" + b, c).html("<em>" + d + "</em>").unbind("click").bind("click", e);
                    else {
                        var g = '<a id="lhgdg_' + x.id + "_" + b + '" class="lhgdg_button" href="javascript:void(0);" target="_self"><em>' + d + "</em></a>",
                ...
                m.cancel = function() {
                    O();
                    if (d) if (x.parent && x.parent.opt.cover) {
                ...
                m.SetCancelBtn = function(b, d) {
                    d = d || m.cancel,
                    a("#lhgdg_" + x.id + "_dgcancelBtn", c)[0] && a("#lhgdg_" + x.id + "_dgcancelBtn", c).html("<em>" + b + "</em>").unbind("click").bind("click", d)
                },
